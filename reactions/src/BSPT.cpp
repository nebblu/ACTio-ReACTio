...
/* Functions for matter bispectrum in GR, fR and nDGP */
/* Based on work in 1808.01120  */

/* Makes use of ODE systems in BSPTN.cpp */
...
// Calculate sigma_8 at some redshift - we rescale input spectrum at z=0 with D_spt calculated using inite (DGP or GR)
...
void BSPT::mypspline(double pars[], bool internal) const{
SPT spt(C, P_L, 1e-2);
...
real BSPT::Bfit(double k1, double k2, double x) const {
  ...
  double F2A,F2B,F2C, pars[6];
    pars[0] = mys8;
    pars[1] = myknl;

    // modified gravity corrections:
    pars[2] = 1.; // for DGP this is 1 - kappa
    pars[3] = (1.- F_spt * 7./2./pow2(D_spt)); // set to DGP values  - lambda

    pars[4] = myneff(k1);
    pars[5] = myneff(k2);
    F2A = F2fit(pars,k1,k2,x);
    pars[4] = myneff(k1);
    pars[5] = myneff(k3);
    F2B = F2fit(pars,k1,k3,-(k2*x+k1)/k3);
    pars[4] = myneff(k2);
    pars[5] = myneff(k3);
    F2C = F2fit(pars,k2,k3,-(k1*x+k2)/k3);
...
// a==1 (GR analytic)
// a=3 (DGP analytic)
real BSPT::Btree(int model, double k1, double k2, double x) const {
  double k3 =  sqrt(pow2(k1) + pow2(k2) + 2.*k2*k1*x);
  ...
  switch (model) {
   ...
   case 3:
    ...
    break;
   default:
    warning("BSPT: invalid indices, model = %d\n", model);
    return 0;
  }
//Tree
...
// a=1 (GR analytic)
// a=3 (DGP analytic)
real BSPT::Bloop(int model, double k1, double k2, double x) const {
  SPT spt(C, P_L, 1e-2);
  double k3 =  sqrt(pow2(k1) + pow2(k2) + 2.*k2*k1*x);
...
// Calculate the 1-loop contributions
double BSPT::Bloopterms(int model, real k1, real k2, real k3, real x) const{
    	real KMAX = QMAXp/k1;
   	  real KMIN = QMINp/k1;
      real tp = 2.*M_PI;
      real c[3] = {KMIN,-1.,0.};
...
// pars: 0 = scale factor, 1= Omega_{m,0}, extpars = model parameters
// model = 1  GR
// model = 2 (f(R) Hu-Sawicki)
// model = 3  nDGP
real BSPT::Btreen(double pars[], double extpars[], int model, double k1, double k2, double x) const {
BSPTN bsptn;
...
switch(model) {
  case 1:
  extpars[0] = 1e-15; // Set Omega_rc = 0 as GR limit.
  bsptn.initnb0_dgp(pars, extpars, ka, xa, kargs);
  ...
  case 2:
  bsptn.initnb0_fofr(pars, extpars, ka, xa, kargs);
  return 2./pow4(dnorm_spt)*(P_L(k1)*P_L(k2)*F1b_k1*F1b_k2*F2b_k12 + P_L(k3)*P_L(k1)*F1b_k1*F1b_k3*F2b_k13 + P_L(k2)*P_L(k3)*F1b_k2*F1b_k3*F2b_k23);
  ...
  default:
   warning("BSPT: invalid indices, model = %e\n", model);
  return 0.;
...
// MG INITIALISATION
// pars- base cosmological params
// extpars - extended Parameters
// epars - error parameters
 static real bloopdgp(const PowerSpectrum& P_L, double pars[], double extpars[], double epars[], double k1, double k2, double x, double k3, double p, double mu, double phi){
   BSPTN bsptn;
   ...
   pka4 = P_L(ka[4]);
     ...
     bsptn.initnb1_dgp(pars, extpars, epars, ka, xa, kargs);
    ...
    return B222+B321a+B321b+B411;
      ...
      static real bloopfr(const PowerSpectrum& P_L, double pars[], double extpars[], double epars[], double k1, double k2, double x, double k3, double p, double mu, double phi){
        BSPTN bsptn;
        double B411,B222,B321a,B321b,pkp,pk1,pk2,pk3,pka1,pka4,pka3;
        ...
        pka4 = P_L(ka[4]);
          ...
          bsptn.initnb1_fr(pars, extpars, epars, ka, xa, kargs);
...
// 1-loop matter bispectrum for various theories: model = 1 GR, 2 fR, 3 DGP
...
double BSPT::Bloopn(double pars[], double extpars[], double epars[], int model, double k1, double k2, double x)const{
      BSPTN bsptn;
      ...
      switch (model) {
        case 1:
         ...
         extpars[0] = 1e-15; // GR limit of DGP
         ...
         loop = prefac*Integrate<3>(bind(bloopdgp,cref(P_L),pars, extpars, epars, k1,k2,x,k3,std::placeholders::_1,std::placeholders::_2,std::placeholders::_3), c, d, epars[0]);
        ...
        case 2:
          ...
          loop = prefac*Integrate<3>(bind(bloopfr,cref(P_L),pars,extpars,epars,k1,k2,x,k3,std::placeholders::_1,std::placeholders::_2,std::placeholders::_3), c, d, epars[0]);
          tree = 2./pow4(dnorm_spt)*(P_L(k1)*P_L(k2)*F1b_1*F1b_2*F2b_12a + P_L(k3)*P_L(k1)*F1b_1*F1b_3*F2b_13a + P_L(k2)*P_L(k3)*F1b_2*F1b_3*F2b_23a);
          }
          return  tree+loop;
